...
import {
	Layout, Typography, Input, Menu, Slider, Row, Col,
	Checkbox, Image, Rate, Button
} from 'antd';
import {
	SearchOutlined, MenuOutlined, MailTwoTone, BellTwoTone, UserOutlined,
	HomeOutlined, GiftOutlined, LoadingOutlined, InboxOutlined, CloseCircleOutlined,CloseCircleFilled,
	StarTwoTone
	...
	const card = (user, index) => {
		return (
			<div key={index} style={{ marginTop: 30 }} className="card1">
				<div className="div1">
					<Title style={{}} level={3} >{user.icon}  {user.text} <Title style={{ marginLeft: 25 }} level={5}>${user.Price}/night</Title></Title>
				</div>
				<Image
					preview={false}
					src={user.image}
				...
				<Title style={{ marginLeft: 20 }} level={5}>{user.text2}</Title>
	  ...
	  useEffect(() => {
			...
			if (search == "") {
				...
				const temp = data1.filter((value) => (value.text.toLowerCase().includes(search.toLowerCase())))
				// setData(temp)
				datax=temp
			}
			if (rate.length == 0) {
			...
			if (text.length == 0) {
				...
				let temp1 = []
				text.map((item1) => {
					const temp = datax.filter((item) => item.text2.toLowerCase() == item1.toLowerCase())
					let temp2 = temp1.concat(temp)
					temp1 = temp2
				})
				datax=temp1
			...
			if (button.length == 0) {
				...
				let temp1 = []
				button.map((item1) => {
					const temp = datax.filter((item) => item.facility.includes(item1.toLowerCase()))
			...
			<Header className="header">
				...
				<Input.Group className="search">
					<Input className="input" size="large"
						value={search}
						placeholder="Search Hotel Name..." prefix={<SearchOutlined />}
			...
			<div style={{ display: 'flex' }}>
					...
					<Content>
						<Menu>
							<SubMenu key="sub1" icon={<HomeOutlined />}>
								<Menu.Item key="1" icon={<LoadingOutlined />} ></Menu.Item>
								<Menu.Item key="2" icon={<LoadingOutlined />} ></Menu.Item>
							</SubMenu>
							<SubMenu key="sub1" icon={<GiftOutlined />}>
								<Menu.Item key="1">Option 1</Menu.Item>
								<Menu.Item key="2">Option 2</Menu.Item>
							</SubMenu>
							<SubMenu key="sub1" icon={<InboxOutlined />}>
								<Menu.Item key="1">Option 1</Menu.Item>
								<Menu.Item key="2">Option 2</Menu.Item>
							</SubMenu>
						</Menu>
				...
				<div className="second">
					<Content style={{ marginLeft: 20 }}>
						{sliderChange && <div>
							<Title  style={{marginTop:25}} level={4}>Applied Filters</Title>
							<div className="filter">
							<Title style={{marginLeft:20}} level={5}>$    {min}-{max}  <Button onClick={() => SetsliderChange(false)} > <CloseCircleFilled />  </Button> </Title>
							</div>
						</div>
						}
						<Title style={{ marginTop: 20}} level={4}>Set Filters</Title>
						...
						<Slider
							...
							value={[min, max]}
						/>
						<Title level={5}>Star Category</Title>
						<Checkbox.Group style={{ width: '100%' }} onChange={onChange}>
							<Col>
								{checkbox_Star.map((user, index) => (
									<Row key={index} span={8}>
										<Checkbox  level={2} value={user.value}>{user.item}</Checkbox>
									</Row>
								))}
							</Col>
						</Checkbox.Group>
						<Title className="checkbox" level={5}>Property Type</Title>
						<Checkbox.Group style={{ width: '100%' }} onChange={onChangeProperty}>
							<Col>
								{checkbox_Property.map((user, index) => (
									<Row key={index} span={8}>
										<Checkbox value={user.item}>{user.item}</Checkbox>
									</Row>
								))}
							</Col>
						</Checkbox.Group>
						<Title className="checkbox" level={5}>Facility</Title>
						<Checkbox.Group style={{ width: '100%'}} onChange={onChangeFacility}>
							<Col>
